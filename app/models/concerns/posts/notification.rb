  ...
  module Notification
    ...
    included do
      ...
      def create_notification_reply(current_user)
        # 親投稿のユーザー全員に通知を送信
        parent_posts = self.ancestors
        parent_posts.each do |parent_post|
          recipient_id = parent_post.user_id
          next if current_user.id == recipient_id # 自分宛の返信は通知しない

          # 返信の通知を作成する
          notification = current_user.sent_notifications.new(
            recipient_id: recipient_id, # 返信の親投稿のユーザーIDを通知の受信者とする
            sender_id: current_user.id, # 通知の送信者
            notifiable: self, # 返信
          ...
          next unless recipient.email_notify_on_reply
