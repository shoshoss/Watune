class Post < ApplicationRecord
  ...
  scope :shared_with_you, ->(current_user, profile_user) {
    direct_to_profile_user = joins(:post_users).where(user_id: current_user.id, post_users: { user_id: profile_user.id, role: 'direct_recipient', approved: true })
    direct_to_current_user = joins(:post_users).where(user_id: profile_user.id, post_users: { user_id: current_user.id, role: 'direct_recipient', approved: true })
    reply_from_current_user = where(user_id: current_user.id, post_reply_id: Post.where(user_id: profile_user.id).select(:id))
    reply_from_profile_user = where(user_id: profile_user.id, post_reply_id: Post.where(user_id: current_user.id).select(:id))
